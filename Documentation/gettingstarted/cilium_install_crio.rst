...
.. tabs::
  .. group-tab:: K8s 1.7
    ...
    .. parsed-literal::
      ...
      $ kubectl create -f \ |SCM_WEB|\/examples/kubernetes/1.7/cilium-crio.yaml
...
Since CRI-O does not automatically detect a new CNI plugin installed you will
need to restart CRI-O daemon to pick the Cilium CNI configuration.
...
And finally, you need to restart Cilium pod so it can re-mount the
``/var/run/crio.sock`` recreated by CRI-O inside the new Cilium pod.
